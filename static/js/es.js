...
$(document).ready(function () {
    ...
    ide.on('dragover dragenter', function (e) {
        e.preventDefault();
        e.stopPropagation();
    });

    // Dosya bırakıldığında
    ide.on('drop', function (e) {
        e.preventDefault();
        e.stopPropagation();
        if (ide.hasClass('content-present')) return;
    ...
    // Tıklamayla dosya seç
    ide.on('click', function () {
        if (!ide.hasClass('content-present')) {
            fileInput.click();
        }
    });
    ...
    fileInput.on('change', function () {
        ...
        if (file && file.type === 'text/plain') {
            readFile(file);
            var idexElement = document.getElementById("idex");
            idexElement.style.display = "none"; // İçerik varsa idex'i gizle
        }
    });
    ...
    function readFile(file) {
        ...
        reader.onload = function (e) {
            ...
            // Ayrı bir textarea varsa, oraya da yaz
           // $('#idetextarea').val(content);
           $('#idetextarea').text(content);
        };
 ...
 // Textarea'nın içeriğini kontrol et
 document.getElementById("idetextarea").addEventListener("input", function() {
    var textareaValue = document.getElementById("idetextarea").value; // Textarea içeriği
    var idexElement = document.getElementById("idex");

    if (textareaValue === "") {
        idexElement.style.display = "block"; // İçerik boşsa idex'i göster
    } else {
        idexElement.style.display = "none"; // İçerik varsa idex'i gizle
    }
});
...
$('#regx').on('input', function () {
    let keyword = $(this).val();
    let $pre = $('#idetextarea');

    // Orijinal metni saklı tut (başta yüklenmiş olmalı)
    let originalText = $pre.data('original');

    // İlk kez yüklüyorsak sakla
    if (!originalText) {
        originalText = $pre.text();
        $pre.data('original', originalText); // orijinali hafızada tut
    }

    if (keyword.trim() === '') {
        $pre.html(originalText); // boşsa orijinali geri koy
        return;
    }

    try {
        // Regex oluştur (tüm eşleşmeleri bul, büyük/küçük harfe duyarsız)
        let regex = new RegExp(keyword, 'gi');
        let highlighted = originalText.replace(regex, function (match) {
            document.getElementById('Regxreplace').style.display = 'block';


/*
            let inputValue = document.getElementById('Regxreplace').value;


            let originalText = $pre.data('original');  // Orijinal metni alıyoruz

            // Regxreplace input'una event listener ekliyoruz
            document.getElementById('Regxreplace').addEventListener('input', function () {
                let inputValue = this.value;  // Regxreplace input'undaki değeri alıyoruz
                let inputElement = document.getElementById('Regxreplace');
            
                // Eğer input boşsa veya input gizli (display: none) ise hiçbir şey yapma
                if (inputValue.trim() === '' || inputElement.style.display === 'none') {
                    return   `<span style="background-color: yellow; color: black;">${match}</span>`;
                }
            
                let regex = new RegExp(keyword, 'gi');  // Regex oluştur (tüm eşleşmeleri bul, büyük/küçük harfe duyarsız)
            
                // Orijinal metni değiştiriyoruz
                let highlighted = originalText.replace(regex, function (match) {
                    // Burada inputValue'yi kullanarak metni değiştirme işlemi
                    return `<span style="background-color: yellow; color: black;">${inputValue}</span>`;
                });
            
                // $pre.data() ile orijinal metni güncelliyoruz
                $pre.data('original', highlighted);  // Orijinal metni güncelliyoruz
            
                // `<pre>` etiketinin içeriğini güncelliyoruz
                document.getElementById('idetextarea').innerHTML = highlighted;  // Güncellenmiş metni pre elementine ekliyoruz
            });
            ...
            */
            ...
            let originalText = $pre.data('original');  // originalText'i veritabanından veya veri kaynağından alıyoruz
            ...
            document.getElementById('Regxreplace').addEventListener('input', function () {
                ...
                // Eğer input boşsa veya input gizli (display: none) ise hiçbir şey yapma
                if (inputValue.trim() === '' || inputElement.style.display === 'none') {
                    return;  // Hiçbir şey yapmadan fonksiyonu sonlandır
                }
                ...
                // Sonucu original metin ile değiştirmek ve idetextarea'ya yerleştirmek
                document.getElementById('idetextarea').innerHTML = highlighted;  
            });
        ...
        $pre.html(highlighted); // HTML olarak yaz
    } catch (e) {
...
var preElement = document.getElementById('idetextarea');
var saveButton = document.getElementById('saveButton');
var originalContent = preElement.textContent; // Başlangıçtaki metni al

preElement.addEventListener('mouseover', function() {
    preElement.contentEditable = true; // Düzenlemeye izin ver
    preElement.style.border = "2px solid blue"; // Stil ekle

    // Eğer içerik değişirse, "Kaydet" butonunu göster
    preElement.addEventListener('input', function() {
        if (preElement.textContent !== originalContent) {
            saveButton.style.display = 'inline-block'; // Kaydet butonunu göster
        }
    });
});

preElement.addEventListener('mouseout', function() {
    preElement.contentEditable = false; // Düzenlemeyi durdur
    preElement.style.border = ""; // Stil kaldır
});

saveButton.addEventListener('click', function() {
    var content = preElement.textContent; // Düzenlenmiş içeriği al

    // Metni bir .txt dosyasına kaydet
    var blob = new Blob([content], { type: 'text/plain' });
    var link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = 'content.txt'; // Dosya adı
